...
def get_extensions():
    if os.environ.get("GROUNDINGDINO_CPU_ONLY") == "1":
        print("Skipping CUDA extension (GROUNDINGDINO_CPU_ONLY=1)")
        return None
    ...
    if CUDA_HOME is not None and (torch.cuda.is_available() or "TORCH_CUDA_ARCH_LIST" in os.environ):
        ...
        if os.name == "nt":
            extra_compile_args["nvcc"] += [
                ...
                "-Xcompiler=/D_DISABLE_EXTENDED_ALIGNED_STORAGE",
                "-Xcompiler=/std:c++17",
            ]
            extra_compile_args["cxx"] += ["/D_ALLOW_COMPILER_AND_STL_VERSION_MISMATCH"]
    else:
